    ...
    if (/^\/?help$/i.test(q)) {
      setHistory((prev) => [
        ...prev,
        {
          ...
          text: [
            "**COMMANDS**",
            "- `analyze <pair>` — structure, momentum and levels",
            "- `news <pair>` — what the wire is saying",
            "- `calendar` — today's scheduled releases",
            "- `correlation` / `usd strength` — cross-asset scorecard",
            "- `levels` — support and resistance",
            "",
            "For the full four-agent Bloomberg report, use the terminal in **Market Analysis**.",
          ].join("\n"),
        },
      ]);
      return;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1.4fr_1fr]">
        <section className="flex flex-col overflow-hidden rounded-2xl border border-[#00ff88]/20 bg-[#0a0a0a] shadow-glow">
          ...
          <div className="max-h-[520px] min-h-[320px] flex-1 space-y-3 overflow-y-auto bg-[#0a0a0a] p-5">
            {hydrated
              ? history.map((m, i) => (
                  <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                    <div
                      className={`max-w-[88%] rounded-lg px-4 py-3 font-mono text-[12.5px] leading-relaxed ${
                        m.role === "user"
          ...
          <div className="border-t border-[#00ff88]/15 p-5">
            ...
            >
              <span className="pl-2 font-mono text-[13px] font-bold text-[#00ff88]" aria-hidden>
                &gt;
              </span>
              <input
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Ask AI...  (type /help)"
